...
-   **뉴스 검색 및 대본 생성**: OpenAI(GPT-4o) 또는 Google Gemini를 사용하여 대화형 뉴스 스크립트 생성 (한국어/영어 지원). 최신순(Recency) 정렬을 통해 가장 따끈따끈한 소식을 우선적으로 수집합니다.
-   **발행 날짜 인지**: 뉴스 기사의 실제 발행 일시를 추출하여 AI에게 전달, 시간 흐름에 맞는 정확한 대본 생성을 보장합니다.
...
0.  **News Collection** (`0_collect_news.js`): 키워드를 바탕으로 네이버 뉴스에서 **최신순**으로 기사를 수집하고, 각 기사의 **발행 일시**를 포함한 컨텍스트를 생성합니다.
