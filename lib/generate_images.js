 ...
 */
export async function searchAndDownloadLogo(entity, domain, outputPath) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            if ((entity || domain) && config.logoOverlay?.enabled) {
                await searchAndDownloadLogo(entity, domain, logoPath);
            }
...
if (process.argv[1] && process.argv[1].includes('generate_images.js')) {
    (async () => {
        try {
